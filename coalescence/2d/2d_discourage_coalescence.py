...
def step(lattice, X, Y, L, action):
    ...
    if action == 0: # jump to the right
        if lattice[nextX][Y] == 1 and (lattice[prevX][Y] == 0 or lattice[X][prevY] == 0 or lattice[X][nextY] == 0):
        ...
        if lattice[prevX][Y] == 1 and (lattice[nextX][Y] == 0 or lattice[X][prevY] == 0 or lattice[X][nextY] == 0):
            reward = -10 # you did the wrong thing buddy
        else:
            lattice[prevX][Y] == 1
        ...
        if lattice[X][nextY] == 1 and (lattice[X][prevY] == 0 or lattice[nextX][Y] == 0 or lattice[prevX][Y] == 0):
            reward = -10 # you did the wrong thing buddy
        else:
            lattice[X][nextY] == 1
        ...
        if lattice[X][prevY] == 1 and (lattice[X][nextY] == 0 or lattice[nextX][Y] == 0 or lattice[prevX][Y] == 0):
            reward = -10 # you did the wrong thing buddy
        else:
            lattice[X][prevY] == 1
...
def do_training(num_episodes, L, Nt):
    ...
    plt.ioff()
    #plt.show()
    plt.savefig("./Training_discourage_2d.png", format="png", dpi=600)
...
if __name__ == '__main__':
    ...
    n_observations = 4      # just NNs at the moment
    ...
    PATH = "./2d_discourage_L" + str(L) + "_NN_params.txt"
        ...
        for t in range(Nt):
            for i in range(newL*newL):
                ...
                if big_lattice[X][Y] != 0:
                    ...
                    #elif big_lattice[newX][newY] == 1 and (other_values[0] == 0 or other_values[1] == 0 or other_values[2] == 0):
    ...
    filename = "2d_discourage_data_L" + str(L) +  "_bigL" + str(newL) + "_runs" + str(runs) + ".txt"
