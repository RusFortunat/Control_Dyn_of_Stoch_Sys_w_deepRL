...
def step(lattice, X, L, action):
    ...
    #if lattice[newX] == 1 and old_state[0] + old_state[1] != 2:
    if lattice[newX] == 1 and lattice[oppositeX] == 0:
        reward = -10
...
def do_training(num_episodes, L, Nt):
        ...
        for t in range(Nt):
            ...
            for i in range(L):
                ...
                if lattice[X] != 0:
                    ...
                    #print("Q values (policy)", Q_values_policy)
                    ...
                    #Q_values_target = target_net(next_state)
                    #print("Q values for the next state (target)", Q_values_target, "\n")
    ...
    plt.ioff()
    #plt.show()
    plt.savefig("./Training_discourage.png", format="png", dpi=600)
...
def plot_score(show_result=False):
    ...
    plt.ylabel('Rewards')
    plt.plot(rewards)
    plt.plot(particle_sum)
    if len(episode_durations) >= 20:
        rewards_t = torch.tensor(rewards, dtype=torch.float)
        means = rewards_t.unfold(0, 20, 1).mean(1).view(-1)
        means = torch.cat((torch.zeros(19), means))
        plt.plot(means.numpy())
        particle_sum_t = torch.tensor(particle_sum, dtype=torch.float)
        means_p = particle_sum_t.unfold(0, 20, 1).mean(1).view(-1)
        means_p = torch.cat((torch.zeros(19), means_p))
        plt.plot(means_p.numpy())
...
if __name__ == '__main__':
    ...
    L = 7                  # Lx, we start with fully occupied lattice
    Nt = 50                # episode duration
    ...
    PATH = "./discourage_NN_params_L" + str(L) + ".txt"
    ...
    if Jesse_we_need_to_train_NN:
        ...
        print("Fraction of mistakes before training: ", mistakes)
        target_net.load_state_dict(policy_net.state_dict())
        optimizer = optim.AdamW(policy_net.parameters(), lr=LR, amsgrad=True)
        memory = ReplayMemory(10000) # the overall memory batch size 
    ...
    print("Fraction of mistakes after training: ", mistakes, "\n")
        ...
        for t in range(Nt):
            for i in range(newL):
                ...
                if big_lattice[X] != 0:
                    ...
                    if big_lattice[newX] == 1 and big_lattice[oppositeX] == 0:
                        print("Investigate! Something is wrong!")

                    if (big_lattice[newX] == 1 and big_lattice[oppositeX] == 1) and t > 500:
                    ...
                    new_state = get_state(big_lattice, newX, L, newL)
            ...
            if newL - perished != n_sum:
                print("Error! Your counting is wrong bruh\n")
    ...
    filename = "discourage_data_L" + str(L) + "_bigL" + str(newL) + "_runs" + str(runs) + ".txt"
